...
export default function CasketsPage() {
  ...
  // Exterior ranges
  const [minEW, setMinEW] = useState("");
  const [maxEW, setMaxEW] = useState("");
  const [minEL, setMinEL] = useState("");
  const [maxEL, setMaxEL] = useState("");
  const [minEH, setMinEH] = useState("");
  const [maxEH, setMaxEH] = useState(""); // <-- FIX: add missing setter

  // Interior ranges
  const [minIW, setMinIW] = useState("");
  const [maxIW, setMaxIW] = useState("");
  const [minIL, setMinIL] = useState("");
  const [maxIL, setMaxIL] = useState("");
  const [minIH, setMinIH] = useState("");
  const [maxIH, setMaxIH] = useState("");
  ...
  const filtered = useMemo(() => {
    ...
    const ewMin = numOrNull(minEW),
      ewMax = numOrNull(maxEW);
    const elMin = numOrNull(minEL),
      elMax = numOrNull(maxEL);
    const ehMin = numOrNull(minEH),
      ehMax = numOrNull(maxEH);
    const iwMin = numOrNull(minIW),
      iwMax = numOrNull(maxIW);
    const ilMin = numOrNull(minIL),
      ilMax = numOrNull(maxIL);
    const ihMin = numOrNull(minIH),
      ihMax = numOrNull(maxIH);
  ...
  async function save(id: number, patch: Partial<Casket>) {
    const res = await fetch(`/api/caskets/${id}`, {
      method: "PATCH",
      body: JSON.stringify(patch),
    });
    if (res.ok) {
      const r = await res.json();
      setRows((prev) => prev.map((p) => (p.id === id ? r : p)));
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-3 items-center">
          <div className="w-80">
            <SearchBar
              value={q}
              onChange={setQ}
              placeholder="Search caskets by name..."
            />
          </div>
      ...
      {/* LANDMARK: Filters slab — explicitly labeled */}
      <HoloPanel>
        ...
        <div className="grid xl:grid-cols-6 md:grid-cols-3 gap-3">
          <div>
            ...
            <select
              className="mt-1 w-full rounded-md bg-white/5 border border-white/10 px-2 py-2 text-sm"
              value={material}
          ...
          <label className="flex items-end gap-2">
            ...
            />
            <span className="text-sm">Jewish only</span>
          </label>

          {/* Exterior */}
          <div>
            <div className="text-xs text-white/60">Exterior Width (in)</div>
            <div className="mt-1 flex gap-2">
              <Input
                placeholder="min"
                value={minEW}
                onChange={(e) => setMinEW(e.target.value)}
              />
              <Input
                placeholder="max"
                value={maxEW}
                onChange={(e) => setMaxEW(e.target.value)}
              />
            </div>
          </div>
          <div>
            <div className="text-xs text-white/60">Exterior Length (in)</div>
            <div className="mt-1 flex gap-2">
              <Input
                placeholder="min"
                value={minEL}
          ...
          <div>
            <div className="text-xs text-white/60">Exterior Height (in)</div>
            <div className="mt-1 flex gap-2">
              <Input
                placeholder="min"
                value={minEH}
                onChange={(e) => setMinEH(e.target.value)}
              />
              <Input
                placeholder="max"
                value={maxEH}
                onChange={(e) => setMaxEH(e.target.value)}
              />
            </div>
          </div>
          ...
          {/* Interior */}
          <div>
            <div className="text-xs text-white/60">Interior Width (in)</div>
            <div className="mt-1 flex gap-2">
              <Input
                placeholder="min"
                value={minIW}
                onChange={(e) => setMinIW(e.target.value)}
              />
              <Input
                placeholder="max"
                value={maxIW}
                onChange={(e) => setMaxIW(e.target.value)}
              />
            </div>
          </div>
          <div>
            <div className="text-xs text-white/60">Interior Length (in)</div>
            <div className="mt-1 flex gap-2">
              <Input
                placeholder="min"
                value={minIL}
                onChange={(e) => setMinIL(e.target.value)}
              />
              <Input
                placeholder="max"
                value={maxIL}
          ...
          <div>
            <div className="text-xs text-white/60">Interior Height (in)</div>
            <div className="mt-1 flex gap-2">
              <Input
                placeholder="min"
                value={minIH}
                onChange={(e) => setMinIH(e.target.value)}
              />
              <Input
                placeholder="max"
                value={maxIH}
                onChange={(e) => setMaxIH(e.target.value)}
              />
            </div>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {filtered.length === 0 && (
          <div className="text-white/60">No matches.</div>
        )}
      </div>
    </div>
  );
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              ...
              value={supplier}
              onChange={(e) =>
                setSupplier(e.target.value ? Number(e.target.value) : "")
              }
            >
              <option value="">—</option>
              {suppliers.map((s) => (
          ...
          <label className="flex items-end gap-2">
            <input
              ...
              className="accent-emerald-400"
              checked={jewish}
              onChange={(e) => setJewish(e.target.checked)}
            />
            <span className="text-sm">Jewish</span>
          </label>

          {/* Exterior */}
          <div className="md:col-span-2 mt-2 text-xs uppercase tracking-wider text-white/50">
            Exterior (inches)
          </div>
          <Input
            placeholder="Width"
            value={extW}
            onChange={(e) => setExtW(e.target.value)}
          />
          <Input
            placeholder="Length"
            value={extL}
            onChange={(e) => setExtL(e.target.value)}
          />
          <Input
            placeholder="Height"
            value={extH}
            onChange={(e) => setExtH(e.target.value)}
          />
          ...
          <Input
            placeholder="Width"
            value={intW}
            onChange={(e) => setIntW(e.target.value)}
          />
          <Input
            placeholder="Length"
            value={intL}
          ...
          <Input
            placeholder="Height"
            value={intH}
            onChange={(e) => setIntH(e.target.value)}
          />
        </div>
    ...
    <HoloPanel>
      {!editing ? (
        <div className="space-y-2">
          ...
          <div className="text-sm text-white/70">
            <span className="font-medium">Exterior:</span> W{" "}
            {row.ext_width_in ?? "—"}″ • L {row.ext_length_in ?? "—"}″ • H{" "}
            {row.ext_height_in ?? "—"}″
          </div>
          <div className="text-sm text-white/70">
            <span className="font-medium">Interior:</span> W{" "}
            {row.int_width_in ?? "—"}″ • L {row.int_length_in ?? "—"}″ • H{" "}
            {row.int_height_in ?? "—"}″
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <select
              className="rounded-md bg-white/5 border border-white/10 px-2 py-2 text-sm"
              value={material}
              onChange={(e) => setMaterial(e.target.value as any)}
            >
              <option value="">—</option>
