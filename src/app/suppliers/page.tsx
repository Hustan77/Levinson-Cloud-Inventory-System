...
export default function SuppliersPage(){
  ...
  const filtered = rows.filter(r=>{
    ...
    return `${r.name} ${r.ordering_instructions ?? ""}`.toLowerCase().includes(q.trim().toLowerCase());
  });
      ...
      {editRow!==null && (
        <Modal onClose={()=>setEditRow(null)} title={editRow.id ? "Edit Supplier" : "Add Supplier"}>
          <form className="space-y-3" onSubmit={async (e)=>{
            ...
            const body:any = { name: fName.trim(), ordering_instructions: fInstr || null };
...
function Modal({ children, onClose, title }: { children: React.ReactNode; onClose: () => void; title: string; }){
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-xl mx-4 rounded-2xl border border-white/10 bg-neutral-900/90 backdrop-blur-xl p-4 shadow-[0_0_40px_rgba(0,0,0,0.45)] max-h-[90vh] overflow-auto">
